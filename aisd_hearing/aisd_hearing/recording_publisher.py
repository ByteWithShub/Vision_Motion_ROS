...
class RecordingPublisher(Node):
    def __init__(self):
        ...
        self.create_timer(1, self.listen)
    ...
    def listen(self):
        ...
        # TODO (not required for Assignment)
        audio_path = os.path.join(
            get_package_share_directory('aisd_hearing'), 'recordings')
        ...
        while True:
            ...
            audio_file = os.path.join(audio_path, f"{current_time}.wav")
...
# Method 1: records audio from a microphone and saves the resulting audio data to a file
...
# Method 2: This function records audio from a specified microphone device and returns the recorded audio data
...
# Method 3: The function add_silence adds silence at both the beginning and the end of an audio data array.
...
# Method 4: to remove leading and trailing silence (blank spots) from audio data
def trim(snd_data):
    def _trim(snd_data):
        snd_started = False
        r = array('h')
...
# Method 5: to determine whether a given segment of audio data is silent or not
def is_silent(snd_data):
    return max(snd_data) < THRESHOLD


# Method 6: to adjust the volume of the audio data
    ...
    if len(args) > 1 and args[1] != "--ros-args":
        if args[1] == "-1":
            ...
            try:
                ...
                sys.exit(1)
    elif len(args) == 1 or (len(args) > 1 and args[1] == "--ros-args"):  # search for default
        for i in range(0, numdevices):
            if p.get_device_info_by_host_api_device_index(0, i).get('maxInputChannels') > 0:
